 ...
 *
 * Severity: per-row override — `empty` → medium, `generic` → low.
 * Confidence: high.
 *
 ...
 *
 * NOTE: the `image-link-missing-alt` sub-case from the plan is intentionally
 * OMITTED here — it requires new extraction columns (`anchor_is_bare_image`,
 * `image_alt_missing`) that are out of scope for this pure-SQL phase. External
 * anchors are ignored (they matter far less for ranking). Findings are emitted on
 * the SOURCE page, deduped by distinct `(source_url, href, anchor_text)`.
 *
 * Detail: `{ href, anchorText, anchorIssue }`.
 */
type AnchorIssue = 'empty' | 'generic';

export const linksAnchorQualityRule: Rule = {
  id: 'links.anchor-quality',
  description: 'Internal links with empty or generic anchor text',
  severity: 'low',
  async run(db, auditId) {
    ...
    return result.rows.map((row): Finding => {
      ...
      const severity: Severity = anchorIssue === 'empty' ? 'medium' : 'low';
      return {
